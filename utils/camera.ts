import p5Types from 'p5'
import { Ray } from './ray'

class Camera {
  p5: p5Types
  pos: p5Types.Vector
  forward: p5Types.Vector
  right: p5Types.Vector
  up: p5Types.Vector
  focalLength: number
  aspectRatio: number
  sensorSize: number
  
  constructor(
    p5: p5Types, pos: p5Types.Vector, forward: p5Types.Vector,
    focalLength: number, aspectRatio: number,
    globalUp? : p5Types.Vector, sensorSize = 0.036) {
    globalUp = globalUp ?? p5.createVector(0, 1, 0)
    this.p5 = p5
    this.pos = pos
    this.forward = forward.normalize()
    this.right = this.forward.copy().cross(globalUp).normalize()
    this.up = this.right.copy().cross(this.forward).normalize()
    this.focalLength = focalLength
    this.aspectRatio = aspectRatio
    this.sensorSize = sensorSize
  }

  generateRay = (nx: number, ny: number) => {
    // 2 * nx - 1,
    // 2 * ny - 1,
    // 0
    // forward = (0, 0, -1)
    // right = (1, 0, 0)
    // up = ()
    const screenCenter = this.forward.copy().mult(this.focalLength).add(this.pos)
    const screenOrigin = screenCenter.copy().sub(
      this.right.copy().mult(0.5 * this.aspectRatio * this.sensorSize)
    ).add(this.up.copy().mult(0.5 * this.sensorSize))
    const targetPosStd = screenOrigin.copy().add(
      this.right.copy().mult(nx * this.aspectRatio * this.sensorSize)
    ).sub(
      this.up.copy().mult(ny * this.sensorSize)
    )
    // alert(screenOrigin)
    return new Ray(this.pos, targetPosStd.copy().sub(this.pos))
  }

}

export { Camera }